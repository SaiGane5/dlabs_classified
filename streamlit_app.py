        ...
        if st.session_state.messages[-1]["role"] != "assistant":
            with st.chat_message("assistant"):
                with st.spinner("Thinking..."):
                    ...
                    # with open('results_dlabs_classified.csv', 'a', newline='') as f:
                    #     writer = csv.DictWriter(f, fieldnames=['Question', 'Time', 'Unigram_Recall', 'Unigram_Precision', 'Bigram_Recall', 'Bigram_Precision', 'Response'])
                    #     writer.writerow({'Question':user_input,'Time':time_taken, 'Unigram_Recall':scores[0]["rouge-1"]["r"], 'Unigram_Precision':scores[0]["rouge-1"]["p"], 'Bigram_Recall':scores[0]["rouge-2"]["r"], 'Bigram_Precision':scores[0]["rouge-2"]["p"], 'Response':response.response})
